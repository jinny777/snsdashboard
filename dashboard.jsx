...
export default function SNSDashboard() {
  ...
  // 콘텐츠 생성 상태
  const [cfTab, setCfTab] = useState("sns");
  const [cfKeyword, setCfKeyword] = useState("");
  const [cfBrand, setCfBrand] = useState("");
  const [cfTone, setCfTone] = useState("professional");
  const [cfPlatforms, setCfPlatforms] = useState(["instagram", "twitter", "facebook", "threads", "youtube"]);
  const [cfResults, setCfResults] = useState(null);
  const [cfGenerating, setCfGenerating] = useState(false);
  const [cfCopied, setCfCopied] = useState({});
  // 브랜드 분석
  const [cfBrandName, setCfBrandName] = useState("");
  const [cfBrandDesc, setCfBrandDesc] = useState("");
  const [cfBrandResult, setCfBrandResult] = useState(null);
  const [cfBrandLoading, setCfBrandLoading] = useState(false);
  // 인스타 캐러셀
  const [cfCarouselKeyword, setCfCarouselKeyword] = useState("");
  const [cfCarouselPlatform, setCfCarouselPlatform] = useState("both");
  const [cfCarouselResult, setCfCarouselResult] = useState(null);
  ...
  const [cfInventionLoading, setCfInventionLoading] = useState(false);
  const [cfInventionTab, setCfInventionTab] = useState("card");
  // 블로그
  const [cfBlogKeyword, setCfBlogKeyword] = useState("");
  ...
  useEffect(() => {
    const loadCredentials = async () => {
      // 1) localStorage에서 먼저 로드 (항상 동작)
      try {
        const localSns = JSON.parse(localStorage.getItem("sns_credentials") || "{}");
        if (Object.keys(localSns).length) {
          setSnsCredentials(prev => {
            const merged = { ...prev };
            Object.keys(localSns).forEach(p => { if (merged[p]) merged[p] = { ...merged[p], ...localSns[p] }; });
            return merged;
          });
        }
        const localSvc = JSON.parse(localStorage.getItem("service_credentials") || "{}");
        if (Object.keys(localSvc).length) {
          setServiceCredentials(prev => {
            const merged = { ...prev };
            Object.keys(localSvc).forEach(s => { if (merged[s]) merged[s] = { ...merged[s], ...localSvc[s] }; });
            return merged;
          });
        }
      } catch (_) {}

      // 2) Supabase에서 로드 (연결된 경우 덮어씀)
      ...
      if (membersData?.length) {
        setMembersList(membersData.map(m => ({
          ...
          joinedAt: m.joined_at, approvalStatus: m.approval_status, role: m.role,
        })));
      }
      if (snsData?.length) {
        const loaded = {};
        ...
        setSnsCredentials(prev => {
          const merged = { ...prev };
          Object.keys(loaded).forEach(p => { merged[p] = { ...prev[p], ...loaded[p] }; });
          return merged;
        });
      }
      if (svcData?.length) {
        ...
        setServiceCredentials(prev => {
          const merged = { ...prev };
          Object.keys(loaded).forEach(s => { merged[s] = { ...prev[s], ...loaded[s] }; });
          return merged;
        });
      }
    };
...
응답 형식: 반드시 아래 JSON 형식으로만 응답하세요. 다른 텍스트 없이 JSON만 출력하세요.
{
  ...
  //  PAGE: 콘텐츠 생성 (공통 헬퍼)
  // ========================
  const cfCallOpenAI = async (systemPrompt, userContent, model = "gpt-4o-mini") => {
    const apiKey = serviceCredentials.openai?.apiKey?.trim();
    if (!apiKey) throw new Error("OpenAI API 키가 없습니다.\n연동 관리 > 서비스 연동 > OpenAI에서 키를 저장해주세요.");
    const resp = await fetch("https://api.openai.com/v1/chat/completions", {
      method: "POST",
      headers: { "Authorization": `Bearer ${apiKey}`, "Content-Type": "application/json" },
      body: JSON.stringify({ model, messages: [{ role: "system", content: systemPrompt }, { role: "user", content: userContent }], temperature: 0.8 }),
    });
    const data = await resp.json();
    if (!resp.ok) throw new Error(data.error?.message || `API 오류 ${resp.status}`);
  ...
  const startInstagramOAuth = () => {
    ...
    const scope = "pages_show_list,pages_read_engagement,instagram_content_publish";
  ...
  const handleCfGenerate = async () => {
    ...
    if (cfPlatforms.length === 0) { alert("플랫폼을 하나 이상 선택해주세요."); return; }
    const apiKey = serviceCredentials.openai?.apiKey?.trim();
    if (!apiKey) { alert("OpenAI API 키가 없습니다.\n연동 관리 > 서비스 연동 > OpenAI에서 키를 저장해주세요."); return; }
    ...
    const systemPrompt = `당신은 SNS 콘텐츠 전문가입니다. 주어진 키워드/주제를 기반으로 각 플랫폼에 최적화된 SNS 콘텐츠를 작성해주세요.
톤앤매너: ${TONE_MAP[cfTone]}${brandInfo}
플랫폼별 요구사항:
${platformList}
응답 형식: 반드시 아래 JSON 형식으로만 응답하세요. 다른 텍스트 없이 JSON만 출력하세요.
{
  ${cfPlatforms.map(p => `"${p}": "해당 플랫폼용 콘텐츠"`).join(",\n  ")}
}`;
    try {
      const resp = await fetch("https://api.openai.com/v1/chat/completions", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system", content: systemPrompt },
            { role: "user", content: `키워드/주제: ${cfKeyword.trim()}` },
          ],
          temperature: 0.8,
        }),
      });
  ...
  const handleCfSaveToContents = (title, platforms, drafts) => {
  ...
  // 브랜드 분석
  const handleCfBrandAnalyze = async () => {
    if (!cfBrandName.trim() || !cfBrandDesc.trim()) { alert("브랜드명과 설명을 입력해주세요."); return; }
    setCfBrandLoading(true); setCfBrandResult(null);
    try {
      const raw = await cfCallOpenAI(
        `당신은 브랜드 전략 전문가입니다. 브랜드 정보를 분석하여 반드시 아래 JSON 형식으로만 응답하세요.
{"usp":"핵심 차별점(2~3문장)","personas":[{"name":"페르소나명","age":"연령대","job":"직업","desc":"특징"}],"painpoints":["페인포인트1","페인포인트2","페인포인트3"],"solutions":["솔루션1","솔루션2","솔루션3"],"tone":"권장 톤앤매너","hashtags":["해시태그1","해시태그2","해시태그3","해시태그4","해시태그5"]}`,
        `브랜드명: ${cfBrandName}\n브랜드 설명: ${cfBrandDesc}`
      );
      const json = raw.replace(/^```json\n?/, "").replace(/\n?```$/, "").trim();
      setCfBrandResult(JSON.parse(json));
    } catch (e) { alert(`분석 실패: ${e.message}`); } finally { setCfBrandLoading(false); }
  };

  // 인스타 캐러셀 / 블로그
  const handleCfCarousel = async () => {
    if (!cfCarouselKeyword.trim()) { alert("키워드를 입력해주세요."); return; }
    setCfCarouselLoading(true); setCfCarouselResult(null);
    setCfCarouselImages({}); setCfCarouselImgLoading({});
    const parseJ = (raw) => JSON.parse(raw.replace(/^```json\n?/, "").replace(/\n?```$/, "").trim());
    try {
      if (cfCarouselPlatform === "both") {
        // 인스타 + 블로그 동시 생성
...
반드시 JSON만 응답: {"titles":["제목1(키워드+클릭유도)","제목2","제목3"],"thumbnailTexts":["썸네일텍스트1","썸네일텍스트2"],"hook":"강력한 오프닝 훅(2문장, 첫 문장에 키워드 포함)","body":"본문 순수텍스트(소제목은 [소제목: 내용] 형식, 이미지 위치는 [IMAGE_1] [IMAGE_2] [IMAGE_3] 표시, 최소 1000자, 자연스러운 문단으로 구성)","imageRecommends":["이미지추천1","이미지추천2","이미지추천3"],"internalLinks":["내부링크주제1","내부링크주제2"],"hashtags":["해시태그1","해시태그2","해시태그3","해시태그4","해시태그5","해시태그6","해시태그7","해시태그8","해시태그9","해시태그10"],"cta":"마무리 CTA 문장"}`,
  ...
  // 발명 아이디어 → 카드뉴스 + 숏츠 스크립트 + 블로그
  const handleCfInvention = async () => {
    const { title, problem, solution, field, target, tone } = cfInvention;
    if (!title.trim() || !problem.trim() || !solution.trim()) { alert("제목, 문제, 해결방법을 입력해주세요."); return; }
    setCfInventionLoading(true); setCfInventionResult(null);
    const TONE_MAP = { professional: "전문적이고 신뢰감 있는", emotional: "감성적이고 스토리텔링", casual: "친근하고 대화체" };
    const context = `발명 제목: ${title}\n해결하는 문제: ${problem}\n해결 방법: ${solution}\n활용 분야: ${field}\n타겟 고객: ${target}\n톤: ${TONE_MAP[tone]}`;
    try {
      const [cardRaw, shortsRaw, blogRaw] = await Promise.all([
        cfCallOpenAI(`당신은 인스타그램 카드뉴스 전문가입니다. 발명 아이디어를 5장 카드뉴스로 만드세요.
반드시 JSON만 응답: {"slides":[{"no":1,"title":"슬라이드제목","body":"본문(2~3줄)","imageDesc":"배경이미지묘사(영문)"},{"no":2,"title":"","body":"","imageDesc":""},{"no":3,"title":"","body":"","imageDesc":""},{"no":4,"title":"","body":"","imageDesc":""},{"no":5,"title":"","body":"","imageDesc":""}],"caption":"인스타 캡션","hashtags":["태그1","태그2","태그3","태그4","태그5"]}`, context),
        cfCallOpenAI(`당신은 유튜브 숏츠 전문가입니다. 발명 아이디어를 조회수 폭발 숏츠 스크립트로 만드세요.
반드시 JSON만 응답: {"ytTitle":"유튜브 제목(검색최적화, 30자 이내)","hook3sec":"처음 3초 훅 문구","hashtags":["#Shorts","태그2","태그3","태그4","태그5"],"scenes":[{"no":1,"type":"훅+문제","script":"나레이션","duration":"3초"},{"no":2,"type":"공감","script":"","duration":"3초"},{"no":3,"type":"해결책소개","script":"","duration":"3초"},{"no":4,"type":"핵심설명","script":"","duration":"5초"},{"no":5,"type":"CTA","script":"댓글·저장 유도","duration":"2초"}]}`, context),
        cfCallOpenAI(`당신은 네이버 인기 블로거입니다. 발명 아이디어를 SEO 최적화 블로그 글로 작성하세요.
반드시 JSON만 응답: {"seoTitle":"SEO 제목","metaDesc":"메타 설명(150자 이내)","body":"본문HTML(H2/H3/ul 포함, 최소 600자)","hashtags":["태그1","태그2","태그3","태그4","태그5"]}`, context),
  ...
  // 월별 콘텐츠 플래닝
  const handleCfPlan = async () => {
    if (!cfPlanKeyword.trim()) { alert("브랜드 또는 주제를 입력해주세요."); return; }
    setCfPlanLoading(true); setCfPlanResult(null);
    try {
      const raw = await cfCallOpenAI(
        `당신은 SNS 콘텐츠 플래너입니다. 한 달 콘텐츠 플랜 30개를 작성하세요.
반드시 JSON만 응답: {"plans":[{"no":1,"keyword":"키워드","platform":"instagram","contentType":"캐러셀/릴스/이미지","title":"콘텐츠 제목","hook":"훅 문구","note":"특이사항"},{"no":2,...}]}  (30개 전부)`,
  ...
  // AI 채팅 편집
  const handleCfChat = async () => {
    if (!cfChatInput.trim()) return;
    if (!cfChatContext.trim()) { alert("먼저 콘텐츠를 생성해주세요."); return; }
    const userMsg = cfChatInput.trim();
    setCfChatMessages(prev => [...prev, { role: "user", content: userMsg }]);
    setCfChatInput(""); setCfChatLoading(true);
    try {
      const apiKey = serviceCredentials.openai?.apiKey?.trim();
      if (!apiKey) throw new Error("OpenAI API 키가 없습니다.");
      const resp = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          model: "gpt-4o-mini",
          messages: [
            { role: "system", content: `당신은 SNS 콘텐츠 편집 전문가입니다. 아래 콘텐츠를 사용자 요청에 따라 수정해주세요.\n\n현재 콘텐츠:\n${cfChatContext}` },
            ...cfChatMessages,
      ...
      setCfChatMessages(prev => [...prev, { role: "assistant", content: reply }]);
    } catch (e) { setCfChatMessages(prev => [...prev, { role: "assistant", content: `오류: ${e.message}` }]); }
  ...
  const renderContentFlow = () => {
    ...
    const toneOptions = [
      { value: "professional", label: "전문적", desc: "신뢰감·권위" },
      { value: "emotional", label: "감성적", desc: "공감·스토리" },
      { value: "casual", label: "캐주얼", desc: "친근·대화체" },
    ];
    const CF_TABS = [
      { id: "sns",       label: "SNS 초안",     icon: "📢" },
      { id: "brand",     label: "브랜드 분석",   icon: "🏷️" },
      { id: "carousel",  label: "캐러셀/블로그", icon: "📱" },
    ...
    const cardStyle = { background: "#fff", borderRadius: 12, border: "1px solid #e2e8f0", padding: 20, marginBottom: 16 };
    const btnPrimary = (loading) => ({ padding: "12px 0", borderRadius: 10, border: "none", background: loading ? "#c7d2fe" : "linear-gradient(135deg,#6366f1,#8b5cf6)", color: "#fff", fontSize: 14, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer", width: "100%" });
    const CopyBtn = ({ text }) => {
      const [copied, setCopied] = useState(false);
      return (
        <button onClick={() => { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
          style={{ padding: "3px 10px", borderRadius: 6, border: "1px solid #e2e8f0", background: copied ? "#f0fdf4" : "#fff", color: copied ? "#10b981" : "#64748b", fontSize: 11, cursor: "pointer" }}>
          {copied ? "✓ 복사됨" : "복사"}
        </button>
      );
    };
    ...
    const LoadingState = () => (
      <div style={{ ...cardStyle, padding: 40, textAlign: "center", color: "#6366f1" }}>
        <div style={{ fontSize: 28, marginBottom: 10 }}>⚡</div>
        <div style={{ fontSize: 14, fontWeight: 600 }}>AI가 생성 중입니다...</div>
      </div>
    );
    return (
      <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#f8fafc", overflowY: "auto" }}>
        {/* 헤더 */}
        <div style={{ padding: "16px 28px 0", borderBottom: "1px solid #e2e8f0", background: "#fff" }}>
          <h2 style={{ margin: "0 0 12px", fontSize: 20, fontWeight: 700, color: "#1a1a2e" }}>⚡ 콘텐츠 생성</h2>
          <div style={{ display: "flex", gap: 4 }}>
            {CF_TABS.map(t => (
              <button key={t.id} onClick={() => setCfTab(t.id)} style={{ padding: "8px 16px", border: "none", borderRadius: "8px 8px 0 0", background: cfTab === t.id ? "#f8fafc" : "transparent", color: cfTab === t.id ? "#6366f1" : "#64748b", fontWeight: cfTab === t.id ? 700 : 400, fontSize: 13, cursor: "pointer", borderBottom: cfTab === t.id ? "2px solid #6366f1" : "2px solid transparent" }}>
                {t.icon} {t.label}
              </button>
            ))}
          </div>
        ...
        {/* OpenAI 연결 상태 배너 */}
        {!serviceCredentials.openai?.apiKey?.trim() ? (
          <div style={{ margin: "16px 24px 0", padding: "12px 18px", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ fontSize: 13, color: "#dc2626" }}>⚠️ <strong>OpenAI API 키가 설정되지 않았습니다.</strong> 콘텐츠 생성을 사용하려면 먼저 API 키를 저장해주세요.</div>
            <button onClick={() => { setActiveMenu("sns-integration"); }} style={{ padding: "6px 14px", borderRadius: 8, border: "none", background: "#dc2626", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", marginLeft: 12 }}>
              연동 관리로 이동 →
            </button>
          </div>
        ) : (
          <div style={{ margin: "16px 24px 0", padding: "10px 18px", background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 10, fontSize: 13, color: "#16a34a" }}>
            ✅ OpenAI API 연결됨 — 콘텐츠 생성 준비 완료
          </div>
        )}
        ...
        <div style={{ padding: 24, flex: 1 }}>
          ...
          {cfTab === "sns" && (
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              <div style={{ flex: "1 1 320px", ...cardStyle, display: "flex", flexDirection: "column", gap: 16 }}>
                <div><label style={labelStyle}>키워드 / 주제 *</label>
                  <textarea value={cfKeyword} onChange={e => setCfKeyword(e.target.value)} placeholder="예) 여름 신상 원피스, 파이썬 자동화..." rows={4} style={{ ...inputStyle, resize: "vertical" }} /></div>
                <div><label style={labelStyle}>브랜드 정보 (선택)</label>
                  <textarea value={cfBrand} onChange={e => setCfBrand(e.target.value)} placeholder="예) 20~30대 여성 패션 브랜드, 미니멀 감성..." rows={2} style={{ ...inputStyle, resize: "vertical" }} /></div>
                <div><label style={labelStyle}>톤앤매너</label>
                  <div style={{ display: "flex", gap: 8 }}>
                    {toneOptions.map(t => <button key={t.value} onClick={() => setCfTone(t.value)} style={{ flex: 1, padding: "8px 6px", borderRadius: 8, border: `1.5px solid ${cfTone === t.value ? "#6366f1" : "#e2e8f0"}`, background: cfTone === t.value ? "#eef2ff" : "#fff", cursor: "pointer" }}><div style={{ fontSize: 12, fontWeight: 700, color: cfTone === t.value ? "#6366f1" : "#374151" }}>{t.label}</div><div style={{ fontSize: 10, color: "#94a3b8" }}>{t.desc}</div></button>)}
                  </div>
                </div>
                <div><label style={labelStyle}>플랫폼 선택</label>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {ALL_CF_PLATFORMS.map(p => { const on = cfPlatforms.includes(p); return <button key={p} onClick={() => setCfPlatforms(prev => on ? prev.filter(x => x !== p) : [...prev, p])} style={{ padding: "5px 12px", borderRadius: 20, border: `1.5px solid ${on ? PLATFORMS[p].color : "#e2e8f0"}`, background: on ? PLATFORMS[p].color : "#fff", color: on ? "#fff" : "#64748b", fontSize: 12, cursor: "pointer" }}>{PLATFORMS[p].icon} {PLATFORM_NAMES[p]}</button>; })}
                  </div>
                </div>
                <button onClick={handleCfGenerate} disabled={cfGenerating} style={btnPrimary(cfGenerating)}>{cfGenerating ? "⚡ 생성 중..." : "✨ SNS 초안 생성"}</button>
              </div>
              <div style={{ flex: "2 1 400px" }}>
                {!cfResults && !cfGenerating && <EmptyState icon="✨" text="키워드를 입력하고 생성 버튼을 눌러주세요" />}
                {cfGenerating && <LoadingState />}
                {cfResults && <>
          ...
          {cfTab === "brand" && (
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              <div style={{ flex: "1 1 300px", ...cardStyle, display: "flex", flexDirection: "column", gap: 14 }}>
                <div><label style={labelStyle}>브랜드명 *</label><input value={cfBrandName} onChange={e => setCfBrandName(e.target.value)} placeholder="예) Antigravity" style={inputStyle} /></div>
                <div><label style={labelStyle}>브랜드 설명 *</label><textarea value={cfBrandDesc} onChange={e => setCfBrandDesc(e.target.value)} placeholder="타겟, 제품/서비스, 특징, 경쟁사 등을 자유롭게..." rows={6} style={{ ...inputStyle, resize: "vertical" }} /></div>
                <button onClick={handleCfBrandAnalyze} disabled={cfBrandLoading} style={btnPrimary(cfBrandLoading)}>{cfBrandLoading ? "⚡ 분석 중..." : "🏷️ 브랜드 분석하기"}</button>
              </div>
              <div style={{ flex: "2 1 400px" }}>
                {!cfBrandResult && !cfBrandLoading && <EmptyState icon="🏷️" text="브랜드 정보를 입력하고 분석을 시작하세요" />}
                {cfBrandLoading && <LoadingState />}
                {cfBrandResult && <>
                  <div style={cardStyle}><div style={{ fontWeight: 700, fontSize: 14, marginBottom: 8, color: "#6366f1" }}>💎 핵심 차별점 (USP)</div><p style={{ margin: 0, fontSize: 13, lineHeight: 1.7, color: "#374151" }}>{cfBrandResult.usp}</p></div>
                  <div style={cardStyle}><div style={{ fontWeight: 700, fontSize: 14, marginBottom: 10, color: "#6366f1" }}>👤 고객 페르소나</div>
                    {(cfBrandResult.personas || []).map((p, i) => <div key={i} style={{ background: "#f8fafc", borderRadius: 8, padding: "10px 12px", marginBottom: 8 }}><div style={{ fontWeight: 600, fontSize: 13 }}>{p.name} · {p.age} · {p.job}</div><div style={{ fontSize: 12, color: "#64748b", marginTop: 4 }}>{p.desc}</div></div>)}
                  </div>
                  <div style={{ display: "flex", gap: 12 }}>
                    <div style={{ ...cardStyle, flex: 1 }}><div style={{ fontWeight: 700, fontSize: 13, marginBottom: 8, color: "#ef4444" }}>😓 페인포인트</div>{(cfBrandResult.painpoints || []).map((v, i) => <div key={i} style={{ fontSize: 12, padding: "4px 0", borderBottom: "1px solid #f1f5f9", color: "#374151" }}>• {v}</div>)}</div>
                    <div style={{ ...cardStyle, flex: 1 }}><div style={{ fontWeight: 700, fontSize: 13, marginBottom: 8, color: "#10b981" }}>✅ 솔루션</div>{(cfBrandResult.solutions || []).map((v, i) => <div key={i} style={{ fontSize: 12, padding: "4px 0", borderBottom: "1px solid #f1f5f9", color: "#374151" }}>• {v}</div>)}</div>
                  </div>
                  <div style={cardStyle}><div style={{ fontWeight: 700, fontSize: 13, marginBottom: 8, color: "#6366f1" }}>🎨 권장 톤앤매너</div><div style={{ fontSize: 13, color: "#374151" }}>{cfBrandResult.tone}</div>
                    <div style={{ marginTop: 10, display: "flex", flexWrap: "wrap", gap: 6 }}>{(cfBrandResult.hashtags || []).map((h, i) => <span key={i} style={{ padding: "3px 10px", borderRadius: 12, background: "#eef2ff", color: "#6366f1", fontSize: 12 }}>{h}</span>)}</div>
                  </div>
                </>}
              </div>
            </div>
          ...
          {cfTab === "carousel" && (
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              <div style={{ flex: "1 1 300px", ...cardStyle, display: "flex", flexDirection: "column", gap: 14 }}>
                <div><label style={labelStyle}>키워드 *</label><input value={cfCarouselKeyword} onChange={e => setCfCarouselKeyword(e.target.value)} placeholder="예) 여름 피부 관리 루틴" style={inputStyle} /></div>
                <div><label style={labelStyle}>콘텐츠 유형</label>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {[{ v: "both", l: "📱+📝 동시 생성" }, { v: "instagram", l: "📱 인스타 카드" }, { v: "blog", l: "📝 블로그" }].map(o => <button key={o.v} onClick={() => setCfCarouselPlatform(o.v)} style={{ flex: 1, padding: "10px 8px", borderRadius: 8, border: `1.5px solid ${cfCarouselPlatform === o.v ? "#6366f1" : "#e2e8f0"}`, background: cfCarouselPlatform === o.v ? "#eef2ff" : "#fff", color: cfCarouselPlatform === o.v ? "#6366f1" : "#374151", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>{o.l}</button>)}
                  </div>
                </div>
                <button onClick={handleCfCarousel} disabled={cfCarouselLoading} style={btnPrimary(cfCarouselLoading)}>{cfCarouselLoading ? "⚡ 생성 중..." : "📱 콘텐츠 생성"}</button>
              </div>
              <div style={{ flex: "2 1 400px" }}>
                {!cfCarouselResult && !cfCarouselLoading && <EmptyState icon="📱" text="키워드를 입력하고 콘텐츠 유형을 선택하세요" />}
                {cfCarouselLoading && <LoadingState />}
                {(cfCarouselResult?.type === "instagram" || cfCarouselResult?.type === "both") && (() => {
                  const igData = cfCarouselResult.type === "both" ? cfCarouselResult.instagram : cfCarouselResult.data;
                  const blogData = cfCarouselResult.type === "both" ? cfCarouselResult.blog : null;
                  const SLIDE_GRADIENTS = [
                    ['#667eea','#764ba2'],['#f093fb','#f5576c'],['#4facfe','#00f2fe'],
                    ['#43e97b','#38f9d7'],['#fa709a','#fee140'],['#a18cd1','#fbc2eb'],
                    ['#1a1a2e','#16213e'],
                  ];
                  const KR_FONT = '"Malgun Gothic","Apple SD Gothic Neo","NanumGothic",sans-serif';
                  ...
                  return <>
                    ...
                    <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 20 }}>
                      {allSlides.map((item, i) => {
                        const [c1, c2] = SLIDE_GRADIENTS[item.gi % SLIDE_GRADIENTS.length];
                        const imgUrl = cfCarouselImages[item.slideKey];
                        const imgLoading = cfCarouselImgLoading[item.slideKey];
                        return (
                          <div key={i} style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "center" }}>
                            <div style={{ width: 240, height: 240, borderRadius: 14, background: imgUrl ? `url(${imgUrl}) center/cover` : `linear-gradient(135deg,${c1},${c2})`, position: "relative", overflow: "hidden", boxShadow: "0 6px 20px rgba(0,0,0,0.2)", cursor: "pointer" }}
                              ...
                              {imgLoading && (
                                <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", zIndex: 5, background: "rgba(0,0,0,0.6)" }}>
                                  <div style={{ color: "#fff", fontSize: 12, textAlign: "center" }}>🎨<br />생성 중...</div>
                                </div>
                              )}
                              <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: "20px 16px", zIndex: 1, textAlign: "center" }}>
                                <div style={{ color: "#fff", fontWeight: 700, fontSize: item.isCover ? 16 : 14, lineHeight: 1.45, textShadow: "0 2px 8px rgba(0,0,0,0.8)", marginBottom: 8 }}>
                                  {item.data?.title}
                                </div>
                                {item.isCover && item.data?.subtitle && (
                                  <div style={{ color: "rgba(255,255,255,0.85)", fontSize: 12, lineHeight: 1.4, textShadow: "0 1px 4px rgba(0,0,0,0.8)" }}>{item.data.subtitle}</div>
                                )}
                                {!item.isCover && item.data?.body && (
                                  <>
                                    <div style={{ width: "70%", height: 1, background: "rgba(255,255,255,0.3)", margin: "4px 0 8px" }} />
                                    <div style={{ color: "rgba(255,255,255,0.88)", fontSize: 11, lineHeight: 1.5, textShadow: "0 1px 4px rgba(0,0,0,0.8)" }}>
                                      {item.data.body.slice(0, 70)}{item.data.body.length > 70 ? "…" : ""}
                                    </div>
                                  </>
                                )}
                              </div>
                    ...
                    {/* 텍스트 편집 */}
                    <div style={cardStyle}>
                      <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 12, color: "#374151" }}>✏️ 텍스트 편집 (수정 후 재다운로드)</div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                          <span style={{ minWidth: 64, fontSize: 12, color: "#6366f1", fontWeight: 700, paddingTop: 9 }}>커버</span>
                          <input value={igData?.cover?.title || ""} onChange={e => {
                            const key = cfCarouselResult.type === "both" ? "instagram" : "data";
                            setCfCarouselResult(prev => ({ ...prev, [key]: { ...prev[key], cover: { ...prev[key].cover, title: e.target.value } } }));
                          }} style={{ ...inputStyle, flex: 1, fontSize: 13 }} placeholder="커버 제목" />
                          <input value={igData?.cover?.subtitle || ""} onChange={e => {
                            const key = cfCarouselResult.type === "both" ? "instagram" : "data";
                            setCfCarouselResult(prev => ({ ...prev, [key]: { ...prev[key], cover: { ...prev[key].cover, subtitle: e.target.value } } }));
                          }} style={{ ...inputStyle, flex: 1, fontSize: 13 }} placeholder="커버 부제목" />
                        </div>
                        {(igData?.slides || []).map((s, i) => (
                          <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                            <span style={{ minWidth: 64, fontSize: 12, color: "#6366f1", fontWeight: 700, paddingTop: 9 }}>슬라이드{s.no}</span>
                            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 4 }}>
                              <input value={s.title} onChange={e => {
                                const key = cfCarouselResult.type === "both" ? "instagram" : "data";
                                setCfCarouselResult(prev => { const slides = [...prev[key].slides]; slides[i] = { ...slides[i], title: e.target.value }; return { ...prev, [key]: { ...prev[key], slides } }; });
                              }} placeholder="제목" style={{ ...inputStyle, fontSize: 13 }} />
                              <textarea value={s.body} onChange={e => {
                                const key = cfCarouselResult.type === "both" ? "instagram" : "data";
                                setCfCarouselResult(prev => { const slides = [...prev[key].slides]; slides[i] = { ...slides[i], body: e.target.value }; return { ...prev, [key]: { ...prev[key], slides } }; });
                              }} rows={2} placeholder="본문" style={{ ...inputStyle, fontSize: 12, resize: "vertical" }} />
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {/* 캡션/해시태그 */}
                    <div style={cardStyle}>
                      <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 8 }}>📝 인스타 캡션 & 해시태그</div>
                      <div style={{ fontSize: 13, lineHeight: 1.7, marginBottom: 10, color: "#374151" }}>{igData?.caption}</div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                        {(igData?.hashtags || []).map((h, i) => <span key={i} style={{ padding: "2px 10px", borderRadius: 12, background: "#fce7f3", color: "#db2777", fontSize: 12 }}>{h}</span>)}
                      </div>
                    </div>
                      ...
                      <div style={{ marginTop: 8 }}>
                        ...
                        <div style={cardStyle}>
                          ...
                          {(blogData.titles || []).map((t, i) => (
                            <div key={i} style={{ padding: "8px 12px", background: "#f8fafc", borderRadius: 8, marginBottom: 6, fontSize: 13, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                              <span style={{ fontWeight: i === 0 ? 600 : 400 }}>{i + 1}. {t}</span>
                              <CopyBtn text={t} />
                            </div>
                          ))}
                        </div>
                        <div style={cardStyle}>
                          <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 6, color: "#6366f1" }}>🖼️ 썸네일 텍스트</div>
                          <div style={{ display: "flex", gap: 8 }}>
                            {(blogData.thumbnailTexts || []).map((t, i) => <div key={i} style={{ flex: 1, padding: "8px 12px", background: "#fef9c3", borderRadius: 8, fontSize: 13, fontWeight: 600 }}>{t}</div>)}
                          </div>
                        </div>
                        <div style={cardStyle}>
                          <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 6, color: "#6366f1" }}>✍️ 오프닝 훅</div>
                          <div style={{ fontSize: 13, lineHeight: 1.8, color: "#374151" }}>{blogData.hook}</div>
                        </div>
                        <div style={cardStyle}>
                        ...
                        <div style={{ display: "flex", gap: 12 }}>
                          <div style={{ ...cardStyle, flex: 1 }}>
                            <div style={{ fontWeight: 700, fontSize: 12, marginBottom: 6, color: "#6366f1" }}>🖼️ 이미지 추천</div>
                            {(blogData.imageRecommends || []).map((v, i) => <div key={i} style={{ fontSize: 12, padding: "3px 0", color: "#374151" }}>• {v}</div>)}
                          </div>
                          <div style={{ ...cardStyle, flex: 1 }}>
                            <div style={{ fontWeight: 700, fontSize: 12, marginBottom: 6, color: "#6366f1" }}>🔗 내부링크 주제</div>
                            {(blogData.internalLinks || []).map((v, i) => <div key={i} style={{ fontSize: 12, padding: "3px 0", color: "#374151" }}>• {v}</div>)}
                          </div>
                        </div>
                        <div style={cardStyle}>
                          <div style={{ fontWeight: 700, fontSize: 12, marginBottom: 8 }}>해시태그</div>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                            {(blogData.hashtags || []).map((h, i) => <span key={i} style={{ padding: "2px 10px", borderRadius: 12, background: "#dcfce7", color: "#059669", fontSize: 12 }}>{h}</span>)}
                          </div>
                        </div>
                      </div>
                    )}
                ...
                {cfCarouselResult?.type === "blog" && <>
                  ...
                  <div style={cardStyle}><div style={{ fontWeight: 700, fontSize: 12, marginBottom: 6, color: "#6366f1" }}>📢 CTA</div><div style={{ fontSize: 13, color: "#374151" }}>{cfCarouselResult.data.cta}</div></div>
                </>}
              </div>
            </div>
          )}
          ...
          {cfTab === "invention" && (
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              <div style={{ flex: "1 1 300px", ...cardStyle, display: "flex", flexDirection: "column", gap: 12 }}>
                {[["title","발명 제목 *","예) 봇 탐지 우회 자동화 라이브러리"],["problem","해결하는 문제 *","예) 기존 자동화 도구는 봇으로 탐지됨"],["solution","해결 방법 *","예) 인간 행동 패턴 모방으로 자연스러운 조작"],["field","활용 분야","예) SNS 마케팅, 데이터 수집"],["target","타겟 고객","예) 마케터, 데이터 분석가"]].map(([k, l, ph]) => (
                  <div key={k}><label style={labelStyle}>{l}</label><input value={cfInvention[k]} onChange={e => setCfInvention(prev => ({ ...prev, [k]: e.target.value }))} placeholder={ph} style={inputStyle} /></div>
                ))}
                <div><label style={labelStyle}>톤</label>
                  <div style={{ display: "flex", gap: 6 }}>
                    {toneOptions.map(t => <button key={t.value} onClick={() => setCfInvention(prev => ({ ...prev, tone: t.value }))} style={{ flex: 1, padding: "7px 4px", borderRadius: 8, border: `1.5px solid ${cfInvention.tone === t.value ? "#6366f1" : "#e2e8f0"}`, background: cfInvention.tone === t.value ? "#eef2ff" : "#fff", color: cfInvention.tone === t.value ? "#6366f1" : "#374151", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>{t.label}</button>)}
                  </div>
                </div>
                <button onClick={handleCfInvention} disabled={cfInventionLoading} style={btnPrimary(cfInventionLoading)}>{cfInventionLoading ? "⚡ 생성 중 (잠시 대기)..." : "💡 콘텐츠 생성 (3종)"}</button>
                <div style={{ fontSize: 11, color: "#94a3b8", textAlign: "center" }}>카드뉴스 + 숏츠 스크립트 + 블로그 동시 생성</div>
              </div>
              <div style={{ flex: "2 1 400px" }}>
                {!cfInventionResult && !cfInventionLoading && <EmptyState icon="💡" text="발명 아이디어 정보를 입력하고 생성하세요" />}
                {cfInventionLoading && <LoadingState />}
                {cfInventionResult && <>
                  <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                    {[["card","📱 카드뉴스"],["shorts","🎬 숏츠 스크립트"],["blog","📝 블로그"]].map(([k, l]) => <button key={k} onClick={() => setCfInventionTab(k)} style={{ flex: 1, padding: "8px", borderRadius: 8, border: `1.5px solid ${cfInventionTab === k ? "#6366f1" : "#e2e8f0"}`, background: cfInventionTab === k ? "#eef2ff" : "#fff", color: cfInventionTab === k ? "#6366f1" : "#374151", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>{l}</button>)}
                  </div>
                  {cfInventionTab === "card" && <>
                    ...
                    <div style={cardStyle}><div style={{ fontWeight: 700, fontSize: 13, marginBottom: 8 }}>🎬 씬 구성</div>
                      {(cfInventionResult.shorts.scenes || []).map((s, i) => (
                        <div key={i} style={{ padding: "8px 10px", background: "#f8fafc", borderRadius: 6, marginBottom: 6, display: "flex", gap: 10, alignItems: "flex-start" }}>
                          <span style={{ minWidth: 20, fontWeight: 700, fontSize: 12, color: "#6366f1" }}>#{s.no}</span>
                          <div><div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 2 }}>{s.type} · {s.duration}</div><div style={{ fontSize: 13 }}>{s.script}</div></div>
                        </div>
                      ))}
                    </div>
                    <div style={cardStyle}><div style={{ fontWeight: 700, fontSize: 13, marginBottom: 6 }}>🏷️ 해시태그</div><div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{(cfInventionResult.shorts.hashtags || []).map((h, i) => <span key={i} style={{ padding: "2px 8px", borderRadius: 12, background: "#ecfdf5", color: "#059669", fontSize: 12 }}>{h}</span>)}</div></div>
                    <div style={{ ...cardStyle, background: "#f0f9ff", border: "1px solid #bae6fd" }}>
                      <div style={{ fontWeight: 700, fontSize: 13, color: "#0284c7", marginBottom: 8 }}>🎬 영상 자동 생성</div>
                      <div style={{ fontSize: 12, color: "#64748b" }}>TTS 나레이션 + Ken Burns 효과 + 배경음악 자동 합성은 백엔드 서버(FFmpeg, Fish Audio)가 필요합니다.</div>
                    </div>
                  </>}
                  {cfInventionTab === "blog" && <>
                    <div style={cardStyle}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span style={{ fontWeight: 700, fontSize: 13, color: "#6366f1" }}>SEO 제목</span><CopyBtn text={cfInventionResult.blog.seoTitle} /></div><div style={{ fontSize: 15, fontWeight: 700 }}>{cfInventionResult.blog.seoTitle}</div></div>
                    <div style={cardStyle}><div style={{ fontWeight: 700, fontSize: 13, marginBottom: 6, color: "#6366f1" }}>메타 설명</div><div style={{ fontSize: 13, color: "#374151" }}>{cfInventionResult.blog.metaDesc}</div></div>
                    <div style={cardStyle}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}><span style={{ fontWeight: 700, fontSize: 13, color: "#6366f1" }}>본문 HTML</span><CopyBtn text={cfInventionResult.blog.body} /></div><textarea value={cfInventionResult.blog.body} readOnly rows={8} style={{ ...inputStyle, background: "#f8fafc", resize: "vertical", fontSize: 12, fontFamily: "monospace" }} /></div>
                    <div style={cardStyle}><div style={{ fontWeight: 700, fontSize: 13, marginBottom: 6 }}>해시태그</div><div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{(cfInventionResult.blog.hashtags || []).map((h, i) => <span key={i} style={{ padding: "2px 8px", borderRadius: 12, background: "#fce7f3", color: "#db2777", fontSize: 11 }}>{h}</span>)}</div></div>
                  </>}
                </>}
              </div>
            </div>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                      ...
                      <tbody>{(cfPlanResult.plans || []).map((p, i) => (
                        <tr key={i} style={{ borderBottom: "1px solid #f1f5f9", background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                          <td style={{ padding: "7px 10px", color: "#94a3b8", fontWeight: 600 }}>{p.no}</td>
                          <td style={{ padding: "7px 10px", fontWeight: 500 }}>{p.keyword}</td>
                          <td style={{ padding: "7px 10px" }}><span style={{ padding: "2px 7px", borderRadius: 10, background: PLATFORMS[p.platform]?.bg || "#f1f5f9", color: PLATFORMS[p.platform]?.color || "#64748b", fontSize: 11 }}>{PLATFORMS[p.platform]?.icon} {p.platform}</span></td>
                          <td style={{ padding: "7px 10px", color: "#64748b" }}>{p.contentType}</td>
          ...
          {cfTab === "chat" && (
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              <div style={{ flex: "1 1 280px", ...cardStyle, display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ fontWeight: 700, fontSize: 14, color: "#1a1a2e" }}>편집할 콘텐츠 붙여넣기</div>
                <textarea value={cfChatContext} onChange={e => setCfChatContext(e.target.value)} placeholder="수정하고 싶은 콘텐츠를 여기에 붙여넣으세요..." rows={10} style={{ ...inputStyle, resize: "vertical", fontFamily: "inherit" }} />
                <button onClick={() => { setCfChatMessages([]); }} style={{ padding: "8px", borderRadius: 8, border: "1px solid #e2e8f0", background: "#fff", color: "#64748b", fontSize: 12, cursor: "pointer" }}>대화 초기화</button>
                <div style={{ fontSize: 11, color: "#94a3b8" }}>예시 요청: "더 짧게 줄여줘", "해시태그 추가해줘", "톤을 더 친근하게 바꿔줘"</div>
              </div>
              <div style={{ flex: "2 1 400px", display: "flex", flexDirection: "column", gap: 0 }}>
                <div style={{ ...cardStyle, flex: 1, minHeight: 300, maxHeight: 500, overflowY: "auto", display: "flex", flexDirection: "column", gap: 10 }}>
                  {cfChatMessages.length === 0 && <div style={{ textAlign: "center", color: "#94a3b8", paddingTop: 40 }}>왼쪽에 콘텐츠를 붙여넣고 수정 요청을 입력하세요 💬</div>}
                  {cfChatMessages.map((m, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                      <div style={{ maxWidth: "80%", padding: "10px 14px", borderRadius: m.role === "user" ? "12px 12px 2px 12px" : "12px 12px 12px 2px", background: m.role === "user" ? "#6366f1" : "#f1f5f9", color: m.role === "user" ? "#fff" : "#1a1a2e", fontSize: 13, lineHeight: 1.6, whiteSpace: "pre-wrap" }}>
                        {m.content}
                      </div>
                    </div>
                  ))}
                  {cfChatLoading && <div style={{ display: "flex", justifyContent: "flex-start" }}><div style={{ padding: "10px 14px", borderRadius: "12px 12px 12px 2px", background: "#f1f5f9", fontSize: 13, color: "#94a3b8" }}>✍️ 작성 중...</div></div>}
                </div>
                <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                  <input value={cfChatInput} onChange={e => setCfChatInput(e.target.value)} onKeyDown={e => e.key === "Enter" && !e.shiftKey && handleCfChat()} placeholder="수정 요청을 입력하세요 (Enter 전송)" style={{ ...inputStyle, flex: 1 }} />
  ...
  const renderSnsIntegration = () => {
    ...
    const handleServiceSave = async (service) => {
      ...
      // localStorage 우선 저장 (Supabase 연결 여부 무관하게 항상 동작)
      try {
        const saved = JSON.parse(localStorage.getItem("service_credentials") || "{}");
        saved[service] = serviceCredentials[service];
        localStorage.setItem("service_credentials", JSON.stringify(saved));
      } catch (_) {}
      // Supabase 연동 시도 (연결된 경우에만)
      ...
      setServiceSaveStatus(prev => ({ ...prev, [service]: "saved" }));
      if (error && !DEMO_MODE) console.warn("Supabase 저장 실패 (로컬에는 저장됨):", error.message);
    };
    ...
    const handleSave = async (platform) => {
      ...
      try {
        const saved = JSON.parse(localStorage.getItem("sns_credentials") || "{}");
        saved[platform] = snsCredentials[platform];
        localStorage.setItem("sns_credentials", JSON.stringify(saved));
      } catch (_) {}
      // Supabase 연동 시도
      ...
      setSnsSaveStatus(prev => ({ ...prev, [platform]: "saved" }));
      if (error && !DEMO_MODE) console.warn("Supabase 저장 실패 (로컬에는 저장됨):", error.message);
    };
